...
//
//paloma rodriguez october 27th 2023 cs405
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // check for injection
  std::string sanitizedSQL = sql;

  // case keyword detection
  std::string lowercaseSQL = sql;
  std::transform(lowercaseSQL.begin(), lowercaseSQL.end(), lowercaseSQL.begin(), ::tolower);

  // check for common keywords
  if (lowercaseSQL.find("or") != std::string::npos || lowercaseSQL.find("and") != std::string::npos)
  {
      // potential injection detected
      std::cout << "Possible SQL Injection! " << sql << std::endl;
      return false;
  }
